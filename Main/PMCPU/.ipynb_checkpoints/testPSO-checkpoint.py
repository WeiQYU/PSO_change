...
def direct_amplitude_distance_refinement(pso_params, dataX, observed_signal, sampFreq, psdHigh, actual_params=None):
    """
    Direct amplitude-based distance parameter refinement with 90%-110% range constraint
    """
    original_r = pso_params['r']
    original_distance_mpc = 10 ** original_r
    
    # Check 90%-110% range constraint
    if actual_params is not None:
        ...
        # Skip refinement if within 90%-110% range
    ...
    try:
        # Generate template signal with current parameters
        ...
        # Determine whether to accept new distance parameter
...
def two_step_matching(params, dataY, psdHigh, sampFreq, actual_params=None, enable_distance_refinement=True):
    ...
    # Classification based on A parameter (PSO threshold)
    if A < 0.01:
...
def crcbqcpsopsd(inParams, psoParams, nRuns, use_two_step=True, actual_params=None, enable_distance_refinement=True):
    """Particle Swarm Optimization main function with enhanced A parameter tracking"""
...
def crcbpso(fitfuncHandle, nDim, **kwargs):
    """Enhanced PSO core algorithm implementation with A parameter tracking"""
    ...
    psoParams = {
        ...
        'track_particles': True  # Enable particle tracking for A parameter analysis
    }
    ...
    returnData = {
        ...
        'bestFitness': np.inf,
        'fitnessHistory': [],
        'particleHistory': [],  # Track particle positions over time
        'aParameterEvolution': [],  # Specifically track A parameter evolution
        'bestParticleHistory': []  # Track best particle evolution
    }
    ...
    # Record initial fitness and particle states
    returnData['fitnessHistory'].append(float(gbest_fitness))
    
    if psoParams['track_particles']:
        returnData['particleHistory'].append(particles.copy())
        returnData['aParameterEvolution'].append(particles[:, 4].copy())  # A parameter is index 4
        returnData['bestParticleHistory'].append(gbest.copy())
    ...
    with tqdm(range(psoParams['maxSteps']), desc=f'Run {psoParams["run"]}', position=0) as pbar:
        for step in pbar:
            ...
            # Record fitness and particle states
            returnData['fitnessHistory'].append(float(gbest_fitness))
            
            if psoParams['track_particles']:
                # Record every few iterations to save memory for A parameter tracking
                if step % 5 == 0 or step < 50:  # Record first 50 iterations and every 5th after
                    returnData['particleHistory'].append(particles.copy())
                    returnData['aParameterEvolution'].append(particles[:, 4].copy())
                    returnData['bestParticleHistory'].append(gbest.copy())
...
def ssrqc(x, params):
    ...
    # Determine lensing usage based on A parameter (PSO threshold)
    A = x[4]
    use_lensing = A >= 0.01
