...
results_dir = "publication_results_lens_30w"
os.makedirs(results_dir, exist_ok=True)

# Set basic plotting parameters (using default fonts)
plt.rcParams.update({
    'axes.linewidth': 1.2,
    'axes.labelsize': 14,
    'axes.titlesize': 16,
...
def setup_parameters(data):
    ...
    pso_config = {
        'popsize': 100,  # 100 particles
        'maxSteps': 3000,  # 3000 iterations
...
def run_bilby_mcmc(data_dict, param_ranges, n_live_points, n_iter):
...
def create_parameter_comparison_table(pso_results, mcmc_results, actual_params, param_ranges):
    """Create a comprehensive parameter comparison table and save as CSV"""
...
def generate_publication_plots(pso_results, mcmc_results, data_dict, comparison, actual_params):
    """Generate publication-quality plots comparing PSO and MCMC results"""
    ...
    try:
        ...
        # Figure 1: Waveform Reconstruction Comparison - Full time domain
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))
        
        # Top panel: Signal comparison - Full time domain
        ax1.plot(t_np, dataY_only_signal_np * 1e21, 'k-', linewidth=2, label='Injected Signal', alpha=0.8)
        ax1.plot(t_np, pso_results['best_signal'] * 1e21, 'r--', linewidth=2, 
                label=f'PSO Reconstruction (SNR = {comparison["signal_quality"]["PSO"]["SNR"]:.1f})', alpha=0.9)
        ax1.plot(t_np, mcmc_results['best_signal'] * 1e21, 'b:', linewidth=2,
                label=f'MCMC Reconstruction (SNR = {comparison["signal_quality"]["MCMC"]["SNR"]:.1f})', alpha=0.9)
        
        ax1.set_ylabel(r'Strain [$10^{-21}$]', fontsize=14)
        ax1.set_title('Gravitational Wave Signal Reconstruction', fontsize=16, fontweight='bold')
        ax1.legend(loc='upper right', fontsize=12)
        ax1.grid(True, alpha=0.3)
        # Show full time domain
        ax1.set_xlim([t_np[0], t_np[-1]])
        
        # Bottom panel: Residuals - Full time domain
        pso_residual = (pso_results['best_signal'] - dataY_only_signal_np) * 1e21
        mcmc_residual = (mcmc_results['best_signal'] - dataY_only_signal_np) * 1e21
        ...
        ax2.set_ylabel(r'Residual [$10^{-21}$]', fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig(f"{results_dir}/figure1_waveform_reconstruction.png", 
        ...
        plt.tight_layout()
        plt.savefig(f"{results_dir}/figure2_parameter_accuracy.png", 
                   bbox_inches='tight', dpi=300)
        plt.close()
        ...
        # Speed improvement
        speed_improvement = comparison['execution_time']['ratio']
        ax4.bar(['Speed Improvement'], [speed_improvement], color='#2ca02c', 
               alpha=0.8, edgecolor='black', linewidth=1.2)
        ax4.set_ylabel('Speedup Factor', fontsize=12, fontweight='bold')
        ax4.set_title('PSO Speed Advantage', fontsize=14, fontweight='bold')
        ax4.grid(True, alpha=0.3, axis='y')
        ax4.text(0, speed_improvement + speed_improvement*0.02,
                f'{speed_improvement:.1f}×', ha='center', va='bottom', 
                fontsize=11, fontweight='bold')

        plt.tight_layout()
        plt.savefig(f"{results_dir}/figure3_performance_metrics.png", 
                   bbox_inches='tight', dpi=300)
        plt.close()

        print(f"Publication-quality figures saved to {results_dir}:")
        print("  - figure1_waveform_reconstruction.png")
...
def main():
    ...
    try:
        ...
        mcmc_results = run_bilby_mcmc(data_dict, param_ranges, n_live_points=500, n_iter=300)

        # Evaluate and compare results
        comparison = evaluate_results(pso_results, mcmc_results, actual_params, data_dict)

        # Create parameter comparison table (CSV only)
        ...
        summary_df = pd.DataFrame({
            ...
            'PSO': [
                ...
                f"{comparison['execution_time']['ratio']:.2f}×",
                f"{comparison['signal_quality']['PSO']['SNR']:.2f}",
                f"{comparison['signal_quality']['PSO']['match_final']:.4f}",
                f"{np.mean([comparison['parameter_errors']['PSO'][p] for p in comparison['parameter_errors']['PSO']]) * 100:.2f}"
            ],
            'MCMC': [
                ...
                f"{np.mean([comparison['parameter_errors']['MCMC'][p] for p in comparison['parameter_errors']['MCMC']]) * 100:.2f}"
            ]
        })
        ...
        print(f"PSO achieves {comparison['execution_time']['ratio']:.1f}× speedup over MCMC")

        # Generate final performance assessment
        pso_wins = 0
        mcmc_wins = 0

        # Compare execution time (PSO should be faster)
        if comparison['execution_time']['PSO'] < comparison['execution_time']['MCMC']:
            pso_wins += 1
        else:
            mcmc_wins += 1

        # Compare SNR (higher is better)
        if comparison['signal_quality']['PSO']['SNR'] > comparison['signal_quality']['MCMC']['SNR']:
            pso_wins += 1
        else:
            mcmc_wins += 1

        # Compare template match (higher is better)
        if comparison['signal_quality']['PSO']['match_final'] > comparison['signal_quality']['MCMC']['match_final']:
            pso_wins += 1
        else:
            mcmc_wins += 1

        # Compare average parameter errors (lower is better)
        pso_avg_error = np.mean([comparison['parameter_errors']['PSO'][p] for p in comparison['parameter_errors']['PSO']])
        mcmc_avg_error = np.mean([comparison['parameter_errors']['MCMC'][p] for p in comparison['parameter_errors']['MCMC']])
        ...
        print(f"\nPerformance Summary:")
        print(f"PSO advantages: {pso_wins}/4 metrics")
        print(f"MCMC advantages: {mcmc_wins}/4 metrics")
        
        if pso_wins > mcmc_wins:
            print("\nConclusion: PSO demonstrates superior overall performance for gravitational wave parameter estimation.")
        elif mcmc_wins > pso_wins:
            print("\nConclusion: MCMC shows better overall accuracy despite computational cost.")
        else:
            print("\nConclusion: PSO and MCMC show comparable performance with different trade-offs.")
