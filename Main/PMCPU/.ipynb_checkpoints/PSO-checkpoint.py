...
def crcbgenqcsig(dataX, r, m_c, tc, phi_c, I, delta_t, use_lensing=True):
...
def direct_amplitude_distance_refinement(pso_params, dataX, observed_signal, sampFreq, psdHigh, actual_params=None):
    ...
    try:
        ...
        template_signal = crcbgenqcsig(
            dataX, original_r, m_c, tc, phi_c, I, delta_t,
        ...
        new_template_signal = crcbgenqcsig(
            dataX, new_r, m_c, tc, phi_c, I, delta_t,
            use_lensing=use_lensing
        )
...
def two_step_matching(params, dataY, psdHigh, sampFreq, actual_params=None, enable_distance_refinement=True):
    ...
    unlensed_signal = crcbgenqcsig(dataX, r, m_c, tc, phi_c, I, delta_t, use_lensing=False)
    ...
    lensed_signal = crcbgenqcsig(dataX, r, m_c, tc, phi_c, I, delta_t, use_lensing=True)
    ...
    # Classification based on I parameter
    if I < 0.01:
        result['is_lensed'] = False
        result['message'] = f"This is an unlensed signal (I = {I:.6f} < 0.01)"
        result['classification'] = "signal"
    else:
        result['is_lensed'] = True
        result['message'] = f"This is a lens signal (I = {I:.6f} >= 0.01)"
    ...
    if enable_distance_refinement:
        ...
        try:
            ...
            if refinement_info['status'] == 'success':
                ...
                if result['is_lensed']:
                    final_signal = crcbgenqcsig(dataX, r, m_c, tc, phi_c, I, delta_t, use_lensing=True)
                else:
                    final_signal = crcbgenqcsig(dataX, r, m_c, tc, phi_c, I, delta_t, use_lensing=False)
...
def crcbqcpsopsd(inParams, psoParams, nRuns, use_two_step=True, actual_params=None, enable_distance_refinement=True):
    ...
    for lpruns in range(nRuns):
        ...
        r, m_c, tc, phi_c, I, delta_t = params
        ...
        if use_two_step:
            ...
            estSig = crcbgenqcsig(inParams['dataX'], r, m_c, tc, phi_c, I, delta_t, use_lensing=use_lensing)
            ...
            if is_lensed:
                lensing_message = f"This is a lens signal (I = {I:.6f} >= 0.01)"
                classification = "lens_signal"
            else:
                lensing_message = f"This is an unlensed signal (I = {I:.6f} < 0.01)"
    ...
    outResults.update({
        ...
        'I': outResults['allRunsOutput'][bestRun]['I'],
...
def ssrqc(x, params):
    ...
    # Determine lensing usage based on I parameter
    I = x[4]
    use_lensing = I >= 0.01
