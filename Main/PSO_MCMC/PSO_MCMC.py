...
results_dir = "PSO_MCMC_lens_signal_result"
...
def run_bilby_mcmc(data_dict, param_ranges, n_live_points=180, n_iter=1700):
...
def generate_pso_all_runs_csv(pso_results, data_dict, actual_params, param_ranges):
    ...
    try:
        ...
        for run_idx, run_result in enumerate(pso_results['all_runs']):
            ...
            # Calculate errors
            distance_error = abs(
                (distance_mpc - actual_params['source_distance']) / actual_params['source_distance']) * 100
...
def generate_comparison_csv(pso_results, mcmc_results, actual_params):
    ...
    try:
        ...
        pso_errors = {
            ...
            'phase': abs(float(pso_results['best_params']['phi_c']) - actual_params['phase'] * np.pi) / (
                        2 * np.pi) * 100,
        ...
        mcmc_errors = {
            'distance': abs(
                (mcmc_distance - actual_params['source_distance']) / actual_params['source_distance']) * 100,
            ...
            'phase': abs(float(mcmc_results['best_params']['phi_c']) - actual_params['phase'] * np.pi) / (
                        2 * np.pi) * 100,
    ...
    try:
        ...
        pso_errors = {
            ...
            'tc': abs((float(pso_results['best_params']['tc']) - actual_params['merger_time']) / actual_params[
                'merger_time']) * 100,
            'phi_c': abs(float(pso_results['best_params']['phi_c']) - actual_params['phase'] * np.pi) / (
                        2 * np.pi) * 100,
            'A': abs((float(pso_results['best_params']['A']) - actual_params['flux_ratio']) / actual_params[
                'flux_ratio']) * 100,
        ...
        mcmc_errors = {
            ...
            'tc': abs((float(mcmc_results['best_params']['tc']) - actual_params['merger_time']) / actual_params[
                'merger_time']) * 100,
            'phi_c': abs(float(mcmc_results['best_params']['phi_c']) - actual_params['phase'] * np.pi) / (
                        2 * np.pi) * 100,
            'A': abs((float(mcmc_results['best_params']['A']) - actual_params['flux_ratio']) / actual_params[
                'flux_ratio']) * 100,
            'delta_t': abs(
        ...
        ax2_bottom.axhline(y=1, color='black', linestyle='--', alpha=0.8, label='Reference line (ratio = 1)',
                           linewidth=2)
        ...
        plt.title(
            'Algorithm Performance Comprehensive Table - Parameters, True Values, Estimates, Errors, Execution Time, SNR',
            fontsize=14, pad=20)
...
def main():
    ...
    try:
        ...
        pso_errors_detailed = {
            'distance': abs(
                (10 ** float(pso_results['best_params']['r']) - actual_params['source_distance']) / actual_params[
                    'source_distance']) * 100,
            'chirp_mass': abs(
                (10 ** float(pso_results['best_params']['m_c']) - actual_params['chirp_mass']) / actual_params[
                    'chirp_mass']) * 100,
            'merger_time': abs((float(pso_results['best_params']['tc']) - actual_params['merger_time']) / actual_params[
                'merger_time']) * 100,
            'phase': abs(float(pso_results['best_params']['phi_c']) - actual_params['phase'] * np.pi) / (
                        2 * np.pi) * 100,
            'flux_ratio': abs((float(pso_results['best_params']['A']) - actual_params['flux_ratio']) / actual_params[
                'flux_ratio']) * 100,
            'time_delay': abs(
                (float(pso_results['best_params']['delta_t']) - actual_params['time_delay']) / actual_params[
                    'time_delay']) * 100
        }

        mcmc_errors_detailed = {
            'distance': abs(
                (10 ** float(mcmc_results['best_params']['r']) - actual_params['source_distance']) / actual_params[
                    'source_distance']) * 100,
            'chirp_mass': abs(
                (10 ** float(mcmc_results['best_params']['m_c']) - actual_params['chirp_mass']) / actual_params[
                    'chirp_mass']) * 100,
            'merger_time': abs(
                (float(mcmc_results['best_params']['tc']) - actual_params['merger_time']) / actual_params[
                    'merger_time']) * 100,
            'phase': abs(float(mcmc_results['best_params']['phi_c']) - actual_params['phase'] * np.pi) / (
                        2 * np.pi) * 100,
            'flux_ratio': abs((float(mcmc_results['best_params']['A']) - actual_params['flux_ratio']) / actual_params[
                'flux_ratio']) * 100,
            'time_delay': abs(
                (float(mcmc_results['best_params']['delta_t']) - actual_params['time_delay']) / actual_params[
                    'time_delay']) * 100
