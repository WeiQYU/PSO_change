...
# 创建详细的信号比较图，包括比值分析（新增功能）
print("\n创建详细的信号比较图，包括比值分析...")

# 获取数据准备绘图
best_run_idx = outResults['bestRun']
best_sig_np = cp.asnumpy(cp.real(outResults['bestSig']))
actual_sig_np = cp.asnumpy(cp.real(dataY_only_signal))
observed_data_np = cp.asnumpy(cp.real(dataY))

# 创建两个子图
fig, axs = plt.subplots(2, 1, figsize=(15, 15), dpi=300, gridspec_kw={'height_ratios': [1, 1]})

# 1. 信号比较图 (Top subplot)
axs[0].plot(t_cpu, observed_data_np, color='gray', alpha=0.5, label='Observed Data (Signal + Noise)')
axs[0].plot(t_cpu, best_sig_np, color='red', lw=1.5, label=f'Best Estimated Signal (Run {best_run_idx + 1})')
axs[0].plot(t_cpu, actual_sig_np, color='blue', lw=1.0, label='Actual Signal (No Noise)')
axs[0].set_ylabel('Strain')
axs[0].set_title(
    f'Bayesian Approach: Balanced Dimension PSO: Signal Comparison: This is a lens signal (A = {best_flux_ratio:.6f} >= 0.01) - CORRECT CLASSIFICATION')
axs[0].legend()
axs[0].grid(False)

# 2. 残差分析图 (Middle subplot)
residual = best_sig_np - actual_sig_np
axs[1].plot(t_cpu, residual, color='green', lw=1.0, label='Residual (Estimate - Actual)')
axs[1].set_ylabel('Strain Difference')
axs[1].set_title(f'Residual Analysis (Correlation: {best_corr:.4f}, SNR: {best_snr_pycbc:.2f})')
axs[1].legend()
axs[1].grid(False)
