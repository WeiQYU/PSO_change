...
import pycbc.inference as infer
import pycbc.waveform as waveform
import pycbc.detector as detector
import pycbc.types.timeseries as ts
from pycbc.workflow import WorkflowConfigParser
from pycbc.inference import models
# Import emcee for direct use
import emcee
# Import time module for runtime tracking
import time
# Import tqdm for progress bar
from tqdm import tqdm

# Import custom PSO optimization function
from PSO_GPU_main import crcbgenqcsig, normsig4psd, innerprodpsd
...
def generate_waveform(dataX, params):
    ...
    Generated waveform
    """
    r = (10 ** params['r']) * 1e6 * pc
    m_c = (10 ** params['m_c']) * M_sun
    tc = params['tc']
    phi_c = params['phi_c']
    A = params['A']
    delta_t = (10 ** params['delta_t'])

    # Use waveform generation function from PSO code
    waveform = crcbgenqcsig(dataX, params['r'], params['m_c'], tc, phi_c, A, delta_t)
    return waveform
...
def setup_mcmc_model(data, time_array, sampFreq, psd, params_init=None):
    ...
    params_init: Initial parameter estimates (optional, from PSO)

    Returns:
    Configured MCMC model
    """
    # Convert data to PyCBC time series
    delta_t = 1.0 / sampFreq
    strain = ts.TimeSeries(data, delta_t=delta_t)
    ...
    class CustomUniform:
        """Custom uniform distribution, replacing PyCBC's Uniform class"""
    ...
    # We use a custom likelihood function to adapt to lensed waveforms
    class LensedGravitationalWaveModel(models.BaseModel):
        """Lensed gravitational wave signal model"""

        name = 'lensed_gw'

        def __init__(self, variable_params, data, time_array, sampFreq, psd):
            super().__init__(variable_params)
            self.data = data
        ...
        def _loglikelihood(self, **params):
            ...
            # Use inner product calculation method from PSO code
...
def run_mcmc(model, prior, nwalkers=200, niterations=5000, params_init=None):
    ...
    if params_init is not None:
        # If there are PSO results, use them as an initial point
        initial_point = {
            'r': params_init['r'],
            'm_c': params_init['m_c'],
            'tc': params_init['tc'],
...
def main():
    ...
    # Load data
    noise_path = 'generate_ligo/noise.mat'
    data_path = 'generate_ligo/data.mat'
    ...
    inParams = {
        ...
        'sampFreq': Fs,
        'psdHigh': psd,
        'rmin': np.array([-2, 0, 0, 0, 0, 0]),
        'rmax': np.array([4, 3, 10, 2 * np.pi, 1, 2]),
    }

    # PSO step (optional)
    # If you want to directly perform MCMC sampling, you can comment out this part
    print("Running PSO optimization to get initial parameter estimates...")
    from PSO_GPU_main import crcbqcpsopsd
    psoParams = {'maxSteps': 1000}  # Use fewer steps, as it's only for initial estimates
    nRuns = 1
    outResults, _ = crcbqcpsopsd(inParams, psoParams, nRuns)

    # Extract initial parameters from PSO results
    init_params = {
        ...
        'A': outResults['allRunsOutput'][0]['A'],
        'delta_t': outResults['allRunsOutput'][0]['delta_t'],
    }

    print("PSO initial parameter estimates:")
    ...
    sampler = run_mcmc(model, prior, nwalkers=100, niterations=2000, params_init=init_params)

    # Analyze MCMC results
    print("Analyzing MCMC results...")
    burn_in = 500  # Number of burn-in samples
    mcmc_results = analyze_mcmc_results(sampler, burn_in=burn_in)
    ...
    plot_results(t, data, sample_signals, sample_params, noise, classification, mcmc_results)
