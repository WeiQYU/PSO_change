...
def classify_signal(snr, flux_ratio, time_delay, total_mass,mismatch):
    flux_threshold = 2 * (snr ** (-2))
    time_threshold = (2 ** (4 / 5) * total_mass * G) / (c ** 3)  # 质量的倒数（用于时间延迟阈值）
    mismatch_threshold = snr ** (-2)
    ...
    if snr < 8:
        ...
        if (flux_ratio >= flux_threshold and time_delay >= time_threshold) or mismatch >= mismatch_threshold:
            ...
            is_lensed = False
        elif time_delay >= time_threshold:
    ...
    return classification, flux_threshold, time_threshold, is_lensed