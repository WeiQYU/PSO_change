...
def two_step_matching(params, dataY, psdHigh, sampFreq, actual_params=None):
    ...
    with new threshold-based approach.
    ...
    # Mismatch threshold for lensing classification
    ...
    # Step 2: If unlensed mismatch > threshold, try lensed model
    if unlensed_mismatch > MISMATCH_THRESHOLD:
        print("Unlensed mismatch exceeds threshold, trying lensed model")
        ...
        if lensed_mismatch < MISMATCH_THRESHOLD:
            result['is_lensed'] = True
            result['message'] = f"This is a lens signal (lensed mismatch {lensed_mismatch:.6f} < threshold {MISMATCH_THRESHOLD})"
            result['classification'] = "lens_signal"
        else:
            result['is_lensed'] = False
            result['message'] = f"This is an unlensed signal (both models exceed mismatch threshold {MISMATCH_THRESHOLD})"
            result['classification'] = "signal"
    else:
        # If unlensed model already has good match, no need to try lensed model
        print(f"Unlensed mismatch {unlensed_mismatch:.6f} below threshold {MISMATCH_THRESHOLD}, classified as unlensed")
        result['is_lensed'] = False
        result['message'] = f"This is an unlensed signal (mismatch {unlensed_mismatch:.6f} < threshold {MISMATCH_THRESHOLD})"
        result['classification'] = "signal"
        
        # Still generate lensed model for comparison purposes
        lensed_signal = crcbgenqcsig(dataX, r, m_c, tc, phi_c, A, delta_t, use_lensing=True)
        lensed_signal, _ = normsig4psd(lensed_signal, sampFreq, psdHigh, 1)
        estAmp = innerprodpsd(dataY_only_signal, lensed_signal, sampFreq, psdHigh)
        lensed_signal = estAmp * lensed_signal
    ...
    if actual_params is not None:
        ...
        actual_r_log10 = cp.log10(actual_params.get('source_distance', 0)) if actual_params.get('source_distance', 0) > 0 else 0
...
def crcbqcpsopsd(inParams, psoParams, nRuns, use_two_step=True, actual_params=None, balanced_pso=True, use_bayesian=True):
    """
    ...
    for lpruns in range(nRuns):
        ...
        if use_two_step:
            ...
            # Execute two-step matching with new threshold-based approach
...
def crcbpso(fitfuncHandle, nDim, **kwargs):
    ...
    **kwargs : dict
    ...
    with tqdm(range(psoParams['maxSteps']), desc=f'Run {psoParams["run"]}', position=0) as pbar:
        for step in pbar:
            ...
            if psoParams['use_adaptive_strategy'] and step % psoParams['adaptation_interval'] == 0 and step > 0:
                ...
                improvement_rate = (returnData['fitnessHistory'][0] - recent_fitness[-1]) / max(abs(returnData['fitnessHistory'][0]), 1e-10)
                ...
                if psoParams['use_adaptive_strategy'] and current_strategy == "exploitation":
                    ...
                    if psoParams['use_dimension_balance']:
                        ...
                        for dim in range(nDim):
                            ...
                            velocities[i, dim] = (w * velocities[i, dim] +
                                               psoParams['c1'] * r1[dim] * (pbest[i, dim] - particles[i, dim]) * dimension_weights[dim] * 1.5 +
                                               psoParams['c2'] * r2[dim] * (lbest[dim] - particles[i, dim]) * dimension_weights[dim] * 0.8)
                    else:
                    ...
                    if psoParams['use_dimension_balance']:
                        ...
                        for dim in range(nDim):
                            ...
                            velocities[i, dim] = (w * velocities[i, dim] +
                                               psoParams['c1'] * r1[dim] * (pbest[i, dim] - particles[i, dim]) * dimension_weights[dim] +
                                               psoParams['c2'] * r2[dim] * (lbest[dim] - particles[i, dim]) * dimension_weights[dim])
                    else:
    ...
    if psoParams['use_dimension_balance'] and len(particles_history) > 0:
        ...
        balance_report = {
            'dimension_weights': cp.asnumpy(dimension_weights).tolist() if isinstance(dimension_weights, cp.ndarray) else dimension_weights,
            'dimension_progress': cp.asnumpy(dimension_progress).tolist() if isinstance(dimension_progress, cp.ndarray) else dimension_progress,
            'dimension_stagnation': cp.asnumpy(dimension_stagnation).tolist() if isinstance(dimension_stagnation, cp.ndarray) else dimension_stagnation
        }
