...
# Updated parameter ranges with A limited to [0, 1]
#                r    mc   tc   phi    A      Δtd
rmin = cp.array([-2.5, 0.2, 0.1, 0, 0.01, 0.1])
rmax = cp.array([3.5, 1.8, 8.0, 2 * np.pi, 1.0, 4.0])  # Changed A upper limit to 1.0
...
# 打印透镜分析结果
print(f"\n============= Lensing Analysis =============")
print(f"Mismatch Threshold: 0.1 (Modified threshold)")
...
ax2.plot(t_cpu, cp.asnumpy(cp.real(training_noise)), 'gray', alpha=0.3, label='Noise')
...
# 计算最佳运行的相位误差
# 生成真实信号和最佳估计的相位演化
def calculate_phase_evolution(signal, fs):
    """使用解析信号方法计算相位演化"""
    dt = 1.0 / fs
    analytic_signal = np.zeros_like(signal, dtype=complex)

    # 简化的Hilbert变换
    signal_fft = np.fft.fft(signal)
    h = np.zeros_like(signal_fft)
    h[0] = 1
    h[1:len(h) // 2] = 2
    h[len(h) // 2] = 1
...
# 计算相位演化
actual_phase, actual_amp = calculate_phase_evolution(bestSignal_only_cpu, Fs)
estimated_phase, estimated_amp = calculate_phase_evolution(bestSig_cpu, Fs)

# 绘制相位差
plt.figure(figsize=(15, 8), dpi=200)
valid_idx = (actual_amp > 0.1 * np.max(actual_amp)) & (estimated_amp > 0.1 * np.max(estimated_amp))
phase_diff = estimated_phase - actual_phase

# 仅在信号足够强的地方绘制相位差（最大幅度的10%以上）
plt.plot(t_cpu[valid_idx], phase_diff[valid_idx], 'b')
plt.xlabel('Time (s)')
plt.ylabel('Phase Difference (radians)')
title = f'Phase Error Between Estimated and Actual Signal (Run {best_run_number})'
if balanced_pso:
    title = f'Balanced Dimension PSO: {title}'
if use_bayesian:
    title = f'Bayesian Approach: {title}'
plt.title(title)
plt.grid(True)
plt.savefig('phase_error_plot.png')
plt.close()
...
if 'model_comparison' in outResults:
    ...
    # 添加阈值线 (for mismatch)
    threshold_value = 10.0  # 10% mismatch threshold (0.1 converted to percentage)
    ax.axhline(y=threshold_value, color='r', linestyle='--', alpha=0.7, 
               label=f'Mismatch Threshold (10%)')
...
for lpruns in range(nRuns):
    ...
    # 不匹配阈值 - 使用新的阈值 0.1
    mismatch_threshold = 0.1  # Fixed threshold
    ...
    print(f"  Mismatch: {run_epsilon:.6f}, Threshold: {mismatch_threshold:.6f}")
    ...
    if 'dimension_analysis' in outResults['allRunsOutput'][lpruns]:
        ...
        if 'overall' in dim_analysis:
            run_result.update({
                ...
                'dimension_exploration_score': dim_analysis['overall']['mean_coverage'] / (1 + dim_analysis['overall']['coverage_imbalance'])
            })
...
columns = [
    ...
    'SNR_pycbc', 'correlation', 'mismatch', 'mismatch_threshold',
